function [stepData,direction,len,Lstep,Rstep] = StepDetection(data)
    %% Step detection
    % Description: Calculates the length and heading of each step taken in 
    ...
    for i_step = 1:size(sindices,1)
        ...
        temp_direction    = data.H(heading_indices(1):heading_indices(2));
            % re-calibrate heading to fit into proper range (-pi to pi)

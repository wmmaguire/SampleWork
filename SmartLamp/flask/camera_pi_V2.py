...
class Camera(object):
    ...
    scanTime        = 0
    occupancy_timer = 0.8 # Time to let camera settle before detecting movement 
    Occupancy_Detection_Mode = True
    refFrame  = None
    occupied  = False
    movepos   = None
    # Hand Gesture info
    handGestTimer  = 0.7  # Time to take between inbetween hand gesture computations
    handDetectTime = 0
    ...
    def set_HandGestureMode(self,state):
        Camera.hand_gesture_mode = state
        if state:
            Camera.handDetectTime    = Camera.last_access

    def set_faceDetectionMode(self,state):
        Camera.faceDetectionMode = state
        if not state:
            Camera.facepos = []
    
    def reset_OccupancyDetectionMode(self):
        Camera.scanTime = Camera.last_access
        Camera.Occupancy_Detection_Mode = False
...
def img_processor(Camera,inputStream):
    ...
    # Occupancy detection
    dt_occ = Camera.last_access - Camera.scanTime
    if dt_occ > Camera.occupancy_timer:
        Camera.Occupancy_Detection_Mode = True
    Camera,image = OccupancyDetection1(Camera,gray,image)
    # Face detection
    if Camera.faceDetectionMode:
        Camera,image = FaceDetection(Camera,gray,image)
    # Hand Gesture mode
    if Camera.skinHist is not None and Camera.hand_gesture_mode:
        dt_hand = Camera.last_access - Camera.handDetectTime
        # Hand Gestures
        if dt_hand > Camera.handGestTimer:
            #print("dt_hand",dt_hand,Camera.handDetectTime)
...
def OccupancyDetection1(Camera,gray,image):
    ...
    if not Camera.Occupancy_Detection_Mode:
        Camera.refFrame = None    
...
#    skinMask    = cv2.bitwise_and(img,img,mask=skinthresh)
    ...
    for i in range(len(contours)):
        ...
        for j in range(defects.shape[0]):
            ...
            if ((angle <= 90) & (angle not in fingerAngles) &
                ...
                # Log Finger data (finger angle/Dir,palm dir)
    ...
    Camera.fingerDir    = list(set(fingerDir))
    return Camera,img

...
class lstm_network():
    ...
    def __init__(self, X, y, model=None, batch_size=1, nb_epoch=300, step_size=1,
                 neurons=1, stacks=1, val_split=0.0, val_data=None, classification=False, opt='rmsprop'):
    ...
    def prepModel(self, classification=False):
        ...
        # Structure model
        model = Sequential()
        for stack in range(0, self.stacks - 1):
            print("Adding lstm stack #%d" % (stack + 1))
            lstm_layer = LSTM(self.neurons, batch_input_shape=input_shape, dropout=0.2, recurrent_dropout=0.2, stateful=True, return_sequences=True)
            model.add(lstm_layer)
        model.add(LSTM(self.neurons, batch_input_shape=input_shape, dropout=0.2, recurrent_dropout=0.2, stateful=True))
        if classification:
    ...
    def fit_lstm(self):
        ...
        # Train (online)
        for i in bar(range(nepochs)):
            # State in the LSTM layer between batches is cleared by default, therefore LSTM must be set to stateful.
    ...
    def forecast_lstm(self, X, batch_size=1):
        """
